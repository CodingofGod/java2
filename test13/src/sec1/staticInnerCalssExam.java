...
class Outer2 {	//외부 클래스
	...
	static void outMethod() { //외부 클래스의 정적 메소드
		//System.out.println("내부 클래스의 인스턴스 변수 : inNum = "+inNum);
		System.out.println("내부 클래스의 정적 멤버 변수 : sInNum = "+Inner.sInNum);
		//System.out.println("내부에서 외부 클래스의 인스턴스 변수 : num = "+num);
		System.out.println("내부에서 외부 클래스의 정적 멤버 변수 : sNum = "+ sNum);
	}
}

public class staticInnerCalssExam {
	public static void main(String[] args) {
		Outer2 out = new Outer2();	//외부 클래스의 인스턴스
		//out.outMethod(); 	//outMethod()는 정적 메소드이므로 객체를 생성하지 않고, 실행
		Outer2.outMethod();	
		//Outer2.Inner.inMethod(); //inMethod()는 인스턴스 메소드이므로 정적 클래스인 Inner실행할수 없음.
