...
class Outer {		//밖에있는 클래스를 외부 클래스라 한다.
	private int num = 10;	//멤버 필드
	private static int sNum = 20; //정적 멤버 필드 =>데이터 공유
	
	public Inner inner; 	//인스턴스 내부 클래스
	public Outer() {	//Outer 생성자를 호출하게 되면, 자동적으로 내부 클레스에 대한 인스턴스 생성
		inner = new Inner();
	}
	
	class Inner {	//클래스 안에 클래스가있는걸 내부클래스라 한다.
		int Num = 1000;
...
public class InnerClassExam {
	public static void main(String[] args) {
		...
		out.inner.inMethod(); 		//타 클래스에서 객체 생성 후 내부 클래스의 
