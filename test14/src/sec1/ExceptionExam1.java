...
//Exception(예외) : 고장(Failure)이나 오류 (Error)과 다르게 개발자의 잘못된 코딩으로 인해 정상적으로 작동이 되지 않는 경우로서 정상적인 상태로 돌아 갈 수 있는 것을 의미한다
//Mismatch(불일치) : 정상적인 결과와 다르게 결과가 표출되는 것으로 프로그램 코딩 상에 해당 기능이나 구조를 적절한 명령을 하지 않는 경우 발생
//예외(Exception) 처리 : 프로그래밍 로직상 에러가 발생하면, 그 이유의 처리 문장은 처리되지 못하고, 실행이 중지되는데
